...
public class CustomerMgr {
	...
	public void inputData (MembershipMgr mm) { // 등록
		...
		int totalCost= 0;
		int totalPoint = 0;
		list.add(new Customer(id, pw, address, phone, totalCost, totalPoint,mm.FindMembership(totalCost)));
    ...
    public void showMember(MembershipMgr mm) {
    	...
    	int num = sc.nextInt();
        for(int i=0; i<list.size(); i++) {
        	if(list.get(i).getMmsh().getGrade().equals(mm.mlist.get(num-1).getGrade())) {
